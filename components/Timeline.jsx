...
function Root({ children }) {
  return (
    <ul className="flex flex-col items-center justify-center py-8 w-2/3">
      {children}
    </ul>
  );
}
...
function Item({ children }) {
  return (
    <div className="grid grid-cols-[15px_1fr] grid-rows-1 items-center gap-x-2 w-full">
      {children}
    </div>
  );
}
...
function Date({ children }) {
  return (
    <span className="text-neutral-600 dark:text-neutral-300 text-sm max-w-80">
      {children}
    </span>
  );
}
...
function Title({ children }) {
  return (
    <h2 className="text-neutral-800 dark:text-neutral-50 text-2xl max-w-80 -ms-[0px] mb-2">
      {children}
    </h2>
  );
}
